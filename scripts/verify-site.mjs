...
try {
  for (const viewport of [{ width: 1920, height: 1080 }, { width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    ...
    assert.match(stateSummary, /1\s+Collected/i, "state summary should count fully collected unread series");
    ...
    assert.match(platformSummary, /AudioBookshelf\s+6/i, "platform summary should expose AudioBookshelf collected titles");
    assert.match(platformSummary, /Missing\s+26/i, "platform summary should count missing titles");
    assert.match(await page.locator("[data-status-distribution]").innerText(), /Read 169 titles[\s\S]*Owned 6 titles[\s\S]*Missing 26 titles/i, "analytics view should hold status distribution");
    ...
    assert.equal(await page.locator("[data-series-stack] > article").count(), 8, "hide-completed should remove fully read rows");
