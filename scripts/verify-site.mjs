...
try {
  for (const viewport of [{ width: 1920, height: 1080 }, { width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    ...
    assert.match(stateSummary, /5\s+Missing/i, "state summary should count missing-heavy series");
    ...
    assert.equal(await page.locator("[data-book-card]").count(), 32, "Audible platform filter should keep unread gaps from Audible-owned series");
    ...
    assert.equal(await page.locator("[data-series-stack] > article").count(), 8, "missing-series view should show only series with gaps");
