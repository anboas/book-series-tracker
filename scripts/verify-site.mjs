...
try {
  for (const viewport of [{ width: 1920, height: 1080 }, { width: 1440, height: 1000 }, { width: 390, height: 844 }]) {
    ...
    await page.waitForFunction(() => document.querySelector("[data-attention-banner]")?.innerText.includes("26 missing"));
    ...
    assert.match(text, /175\/201 read/i);
    assert.doesNotMatch(text, /6\s+OWNED/);
    ...
    assert.match(text, /The Shattered Path/);
    assert.match(text, /The Path of Ascension 12/);
    ...
    assert.doesNotMatch(stateSummary, /1\s+Collected/i, "state summary should not count Defiance as collected-only after read correction");
    const platformSummary = await page.locator("[data-platform-summary]").innerText();
    assert.match(platformSummary, /Owned audio\s+175/i, "platform summary should count audio-owned titles");
    assert.match(platformSummary, /Audible\s+169/i, "platform summary should count Audible titles");
    assert.match(platformSummary, /AudioBookshelf\s+6/i, "platform summary should expose AudioBookshelf read titles");
    assert.match(platformSummary, /Missing\s+26/i, "platform summary should count missing titles");
    assert.match(await page.locator("[data-status-distribution]").innerText(), /Read 175 titles[\s\S]*Missing 26 titles/i, "analytics view should hold status distribution");
    assert.doesNotMatch(await page.locator("[data-status-distribution]").innerText(), /Owned 6 titles/i, "analytics view should not show owned-only Defiance titles");
    ...
    assert.match(await page.locator("[data-next-unread-strip]").innerText(), /26 missing/i, "attention banner should summarize unread gaps");
    ...
    assert.match(await page.locator("[data-primary-lens-bar]").innerText(), /All[\s\S]*201[\s\S]*Owned audio[\s\S]*175[\s\S]*Unread[\s\S]*26[\s\S]*Next up[\s\S]*5/i, "primary lens bar should summarize core reading modes");
    ...
    assert.equal(await page.locator("[data-book-card]").count(), 26, "Audible platform filter should keep unread gaps from Audible-owned series");
